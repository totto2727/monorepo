import { createClient, type GetListRequest, MicroCMSListResponse as MicroCMSListResponse_, GetListDetailRequest, MicroCMSImage, MicroCMSListContent as MicroCMSListContent_, MicroCMSQueries } from "microcms-js-sdk";
...
export type MicroCMSCustomField<T extends string, U> = U extends Record<string, MicroCMSPrimitiveField | MicroCMSExtendedField | MicroCMSRepeatedField<infer _>> ? U & { fieldId: T } : never

export type MicroCMSContent<T> = T extends Record<string, MicroCMSPrimitiveField | MicroCMSExtendedField | MicroCMSRepeatedField<infer _> | MicroCMSCustomField<infer __, infer ___>> ? T : never

export type MicroCMSListContent<T> = T extends MicroCMSContent<infer _> ? T & MicroCMSListContent_ : never

export type MicroCMSQueriesWithoutFields = Omit<MicroCMSQueries, 'fields'>
export type GetListRequestWithoutFields = Omit<GetListRequest, 'queries'> & { queries?: Omit<MicroCMSQueries, 'fields'> }
export type GetListDetailRequestWithoutFields = Omit<GetListDetailRequest, 'queries'> & { queries?: Omit<MicroCMSQueries, 'fields'> }


export type MicroCMSListResponse<T> = Omit<MicroCMSListResponse_<T>, 'contents'> & {
    contents: T[];
}
